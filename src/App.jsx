...
export default function App(){
  const [currentUser, setCurrentUser] = useState(null); // staffオブジェクト or null
      ...
      <div style={{background:C.paper,borderRadius:20,padding:"28px 24px",width:"100%",maxWidth:360,boxShadow:"0 8px 32px rgba(45,26,14,0.12)"}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input
            ...
            disabled={locked}
            style={{width:"100%",padding:"11px 12px",borderRadius:10,border:`1.5px solid ${error?"#ef4444":C.border}`,background:C.bg,fontFamily:"inherit",fontSize:14,color:C.ink,outline:"none",boxSizing:"border-box"}}
          />
        </div>
        <div style={{marginBottom:20}}>
          ...
          <div style={{position:"relative"}}>
            <input
              ...
              placeholder="パスワード"
              disabled={locked}
              style={{width:"100%",padding:"11px 40px 11px 12px",borderRadius:10,border:`1.5px solid ${error?"#ef4444":C.border}`,background:C.bg,fontFamily:"inherit",fontSize:14,color:C.ink,outline:"none",boxSizing:"border-box"}}
            />
...
//  管理者レイアウト
// ═══════════════════════════════════════════════════
function AdminLayout({tab,setTab,staff,shifts,getShift,setShift,attendance,wages,setWages,getShiftByDate,showToast,now}){
  const TABS=[{id:"shift",icon:"📅",label:"シフト入力"},{id:"punch",icon:"⏱",label:"打刻"},{id:"compare",icon:"🔍",label:"照合"},{id:"wage",icon:"💴",label:"時給設定"}];
  const tabStyle=(active)=>({flex:1,padding:"11px 4px 9px",border:"none",cursor:"pointer",background:active?C.paper:"transparent",borderBottom:active?`3px solid ${C.accent}`:"3px solid transparent",color:active?C.accent:C.muted,fontFamily:"inherit",fontSize:11,fontWeight:active?700:400,transition:"all 0.15s"});
  function getAtt(staffId,date){ return attendance[staffId]?.[dateKey(date)]||{}; }
  function punchIn(staffId){} // 管理者画面では打刻不可（表示のみ）
  function punchOut(staffId){}
  return (
    <>
      <nav style={{display:"flex",background:"#f5e9d6",borderBottom:`2px solid ${C.border}`}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={tabStyle(tab===t.id)}><div style={{fontSize:16}}>{t.icon}</div>{t.label}</button>)}
      </nav>
      <main style={{maxWidth:820,margin:"0 auto",padding:"18px 14px 60px"}}>
...
const PB=bg=>({width:"100%",padding:"12px",background:bg,color:bg==="#fffaf3"||bg==="#fee2e2"?"#2d1a0e":"#fffaf3",border:"none",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"block"});
